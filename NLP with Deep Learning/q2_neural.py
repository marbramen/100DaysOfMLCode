...
def forward_backward_prop(X, labels, params, dimensions):
    ...
    gradz2 = y - labels
    #   print("gradz2.shape" + str(gradz2.shape))

    gradW2 = np.dot(h.T, gradz2)
    gradb2 = np.sum(gradz2, axis=0, keepdims=True)
    gradW1 = np.dot(X.T, sigmoid_grad(h)*np.dot(gradz2, W2.T))
    gradb1 = np.sum(sigmoid_grad(h) * np.dot(gradz2, W2.T), axis=0, keepdims=True)
